...
QUADIO_APP = 'quadio_app'
GCM_API_KEY = ENV['GOOGLE_CLOUD_MESSAGING_API']
NOTIFICATION_TYPES = {
  like: 'like',
  comment: 'comment',
  post: 'post',
...
begin
  unless Rails.env.test?
    return unless ActiveRecord::Base.connection.table_exists? 'rpush_apps'
    return if GCM_API_KEY.blank? || Rpush::Gcm::App.exists?(name: QUADIO_APP)

    app = Rpush::Gcm::App.new
    app.name = QUADIO_APP
    app.auth_key = GCM_API_KEY
    app.connections = 1
    app.save!
  end
